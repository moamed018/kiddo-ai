...
// import GetStoryFromInput from "../api/getStoryFromInput";
...
import generateImage from "../api/getImageGemini";
...
export default function NewStory() {
    ...
    const [images, setImages] = useState<ImagesObject | null>();
    ...
    const createStoryFromUserInput = async () => {
        setTextInput("");
        setTextOptions([]);
        setStatusText(null);
        setLoadingValue(0);
        ...
        if (!textInput) {
            showToast(t("Please enter a title for the story"), {
                type: "warning",
            });
            return;
        }
        ...
        try {
            setBlocked(true);
            setStatusText({
                text: t("Creating story..."),
                color: valuesState?.textColor || "",
            });
            setLoading(true);
            setLoadingValue(0);
            const story = JSON.parse(
                (await GetStoryFromInputGemini({
                    input: textInput,
                    lang: textLang,
                    size: textSize,
                    withImages: textOptions.includes("images"),
                })) || ""
            );
            if (story) {
                setNewStory(story);
                setStatusText({
                    text: t("Story created successfully"),
                    color: "#0f0",
                });
                setLoadingValue(
            ...
            if (story?.scenes && textOptions.includes("images")) {
                setStatusText({
                    text: t("Creating story images..."),
                    color: "#0f0",
                });
                    ...
                    async (
                        ...
                        if (image) {
                            ...
                            setStatusText({
                                text:
                                    t("Done create image") +
                                    ` (${scene.scene_number} / ${story.scenes.length})`,
                                color: "#0f0",
                            });
                        } else {
                            setStatusText({
                                text:
                                    t("Error when creating image") +
                                    ` (${scene.scene_number} / ${story.scenes.length})`,
                                color: "#f00",
                            });
                        }
                ...
                if (story?.scenes?.length === images?.length) {
                    setStatusText({
                        text: t("Story Images created successfully"),
                        color: "#0f0",
                    });
                }
                setLoadingValue(textOptions.length === 2 ? 100 : 77);
            }
        } catch (error) {
            setStatusText({ text: t("Failed to create story"), color: "#f00" });
            console.error(error);
            setBlocked(false);
        } finally {
            setLoading(false);
                        ...
                        >
                            <div className="flex flex-col card justify-start p-2 gap-2 items-start sm:flex-row sm:gap-4">
                                ...
                                <Button
                                    ...
                                    onClick={createStoryFromUserInput}
                                >
                                    {t("createNewStory")}
                                </Button>
                            </div>
                                        ...
                                        <div className="md:px-12 mt-4 px-0 sm:px-4">
                                            {NewStory?.scenes?.map(
                                                (scene, index) => (
                                                    <div
                                                        key={index}
                                                        className={`flex items-start mt-10 mb-2 ${
                                                            images ||
